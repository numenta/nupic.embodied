...
from https://github.com/pathak22/exploration-by-disagreement
"""
...
class CloudpickleWrapper(object):
    """
    Use cloudpickle to serialize contents (otherwise multiprocessing tries using pickle)
    """
...
class ShmemVecEnv(VecEnv):
    ...
    def _decode_obses(self, obs):
        ...
        for i, shape in enumerate(self.obs_shapes):
            ...
            o = [
                np.frombuffer(b.get_obj(), dtype=self.obs_dtypes[i]).reshape(shape)
                for b in bufs
            ]
...
def _subproc_worker(pipe, parent_pipe, env_fn_wrapper, obs_buf, obs_shape):
    ...
    def _write_obs(obs):
        ...
        for o, b, s in zip(obs, obs_buf, obs_shape):
            dst = b.get_obj()
            dst_np = np.frombuffer(dst, dtype=_CT_TO_NP[dst._type_]).reshape(
                s
